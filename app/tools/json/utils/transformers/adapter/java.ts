...
const javaTransformer: TransformerFn = async (value) => {
  ...
  kotlinTransformationLines.forEach((line: string) => {
    ...
    if (line === ')') {
      ...
      const variable: string = processedLine.substring(variableStartIndex, variableEndIndex);
      const typeStartIndex = processedLine.indexOf(':') + 2;
      let type: string = processedLine.substring(typeStartIndex, processedLine.length - 1);
...
export default javaTransformer;
